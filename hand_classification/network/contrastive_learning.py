...
if __name__ == '__main__':
    ...
    transfer_learning.model_name = f"{transfer_learning.base_model}_contrastive"

    model_freeze, model_train = transfer_learning.create_model()

    train_data, test_data = \
        transfer_learning.load_data(train_dir_path=f"/home/{USERNAME}/Datasets/ASL/train",
                                    test_dir_path=f"/home/{USERNAME}/Datasets/ASL/{folder}")
    ...
    history = encoder_with_projection_head.fit(
        x=features_train, y=labels_train, batch_size=256, epochs=50, verbose=2,
        # callbacks=[callback]
    )

    classifier = create_classifier(encoder_with_projection_head, trainable=False)

    history = classifier.fit(x=features_train, y=np.array(labels_categorical), batch_size=256, epochs=50,
                             verbose=2, )
    ...
    model.save(ROOT_DIR + f"/hand_classification/models/InceptionV3_contrastive/myModel")
